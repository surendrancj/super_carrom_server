...
var onGenerateToken = async (userdata) => await import_JWT.JWT.sign(userdata);
var onHashPassword = async (password) => import_Hash.Hash.make(password);
var getDirname = () => typeof __dirname !== "undefined" ? __dirname : import_path.default.dirname((0, import_url.fileURLToPath)(import_meta.url));
var htmlTemplatePath = [
  import_path.default.join(process.cwd(), "html"),
  import_path.default.join(getDirname(), "..", "html")
...
var auth = {
  ...
  Please give feedback and report any issues you may find at https://github.com/colyseus/colyseus/issues/660
    ...
    const originDetector = function(req, _, next) {
      if (!auth.backend_url) {
        auth.backend_url = req.protocol + "://" + req.get("host");
      }
      if (!import_oauth.oauth.defaults.origin) {
        import_oauth.oauth.defaults.origin = auth.backend_url;
      }
    ...
    router.get("/confirm-email", async (req, res) => {
      if (req.query.success || req.query.error) {
        const html = await import_promises.default.readFile(import_path.default.join(htmlTemplatePath, "address-confirmation.html"), "utf-8");
        return res.end(html);
      }
