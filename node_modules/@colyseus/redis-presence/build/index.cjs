...
var RedisPresence = class {
  constructor(options, clusterOptions) {
    this.subscriptions = new import_events.default();
    this.handleSubscription = (channel, message) => {
      this.subscriptions.emit(channel, JSON.parse(message));
    };
    if (options instanceof import_ioredis.Redis || options instanceof import_ioredis.Cluster) {
      this.pub = options;
      ...
      this.sub = new import_ioredis.Redis(options);
      this.pub = new import_ioredis.Redis(options);
    }
  ...
  channels(pattern = "*") {
    return this.pub.pubsub("CHANNELS", pattern);
  }
  async exists(key) {
