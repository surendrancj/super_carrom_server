...
var import_grant = __toESM(require("grant"), 1);
...
${providerUrl ? `<hr/><p><small><em>(Get your keys from <a href="${providerUrl}" target="_blank">${providerUrl}</a>)</em></small></p>` : ""}
      ...
      router.use((req, res, next) => {
        const dynamicOrigin = oauth.defaults.origin || import_auth.auth.backend_url;
        if (dynamicOrigin) {
          res.locals.grant = {
      ...
      router.use(import_grant.default.default.express(config));
